 ...
 * rodKbEILogger.cpp
 * 
 * Concrete data logging class that records the curvatures/bending stiffnesses for each rod segment.
 * 
 * Copyright 2020 Andrew P. Sabelhaus and Soft Machines Lab at CMU
 */

#include "rodKbEILogger.h"

// Constructor: the parent does everything for us
rodKbEILogger::rodKbEILogger(std::string fileNamePrefix, std::string logfile_base, std::ofstream& df, shared_ptr<world> w, int per) : 
                            worldLogger(fileNamePrefix, logfile_base, df, w, per)
{
    if(verbosity >= 2) {
        std::cout << "Constructing a rodKbEILogger..." << std::endl;
    }
}

// destructor does nothing
rodKbEILogger::~rodKbEILogger()
{
}

// Redefine the parent's aux setup function.
void rodKbEILogger::setupHelper()
{
    if(verbosity >= 1) {
        std::cout << "Setting up a rodKbEILogger..." << std::endl;
    }
...
std::string rodKbEILogger::getLogHeader()
{
    // dynamically create header based on number of vertices.
    std::ostringstream hdr;
    hdr << "rodKbEILogger.,Columns are limb_property_vert." << std::endl << "Simulation time (sec),";
    ...
    for(std::size_t i=0, max = vert_nums.size(); i < max; i++){
        hdr << i << "_" << "kappaBar_" << vert_nums[i] << "," << i << "_" << "EI_" << vert_nums[i] << ",";
    }
...
std::string rodKbEILogger::getLogData()
{
    ...
    for(std::size_t i=0, max = vert_nums.size(); i < max; i++){
        ...
        logdata << m_world_p->getRodP()->kappaBar[vert_nums[i]] << "," << m_world_p->getRodP()->EI[vert_nums[i]] << ",";
    }
    // base class appends an endline
    // logdata << std::endl;
    return logdata.str();
